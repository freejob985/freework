                ...
                <div class="mb-3">
                    ...
                    <div class="card project-card">
                        <div class="card-header border-bottom-0">
                            <div>
                                <span
                                    class="badge badge-primary badge-inline badge-md">{{ single_price($bidded_project->project->price) }}</span>
                            </div>
                            <div>
                                @if (\App\Models\ProjectUser::where('project_id',
                                $bidded_project->project_id)->where('user_id', Auth::user()->id)->first() != null)
                                <span
                                    class="badge badge-success badge-inline badge-md">{{ translate('Hired You') }}</span>
                                @elseif(\App\Models\ProjectUser::where('project_id',
                                $bidded_project->project_id)->first() != null)
                                <span
                                    class="badge badge-secondary badge-inline badge-md">{{ translate('Someone Else Hired') }}</span>
                                @else
                                <span
                                    class="badge badge-secondary badge-inline badge-md">{{ translate('Not Hired Yet') }}</span>
                                @endif
                            </div>
                            < </div> <div class="card-body pt-1">
                                <h5 class="h6 fw-600 lh-1-5">
                                    <a href="{{ route('project.details', $bidded_project->project->slug) }}"
                                        class="text-inherit" target="_blank">{{ $bidded_project->project->name }}</a>
                                </h5>
                                <ul class="list-inline opacity-70 fs-12">
                                    ...
                                    <li class="list-inline-item">
                                        <a href="#" class="text-inherit">
                                            ...
                                            <span>@if ($bidded_project->project->project_category != null)
                                                {{ $bidded_project->project->project_category->name }} @else
                                                {{ translate('Removed Category') }} @endif</span>
                                        </a>
                                    </li>
                                    <li class="list-inline-item">
                        ...
                        <div class="card-footer">
                            <div class="d-flex align-items-center">
                                <a href="{{ route('client.details', $bidded_project->project->client->user_name)}}"
                                    class="d-flex mr-3 align-items-center text-reset">
                                    <span class="avatar avatar-xs overflow-hidden">
                                        <img class="img-fluid rounded-circle"
                                            src="{{ custom_asset($bidded_project->project->client->photo) }}">
                                    </span>
                                    <div class="pl-2">
                                        ...
                                        <div class="">
                                            ...
                                            <span class="opacity-50">
                                                ({{ getNumberOfReview($bidded_project->project->client->id) }}
                                                {{ translate('Reviews') }})
                                            </span>
                                        </div>
                                    </div>
