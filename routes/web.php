...
Route::post('/Bids/Modification', 'Developer@Bids_Modification')->name('Bids.Modification');
...
Route::group(['middleware' => ['user', 'packagePurchased']], function () {
    ...
    Route::get('/profile-settings', 'ProfileController@user_profile')->name('user.profile')->middleware(['Maintenance']);;
...
Route::group(['middleware' => ['auth', 'client', 'packagePurchased']], function () {
    ...
    Route::get('/my-open-projects', 'ProjectController@my_open_project')->name('projects.my_open_project')->middleware(['Maintenance']);
    Route::get('/project-bids/{slug}', 'ProjectController@project_bids')->name('project.bids')->middleware(['Maintenance']);
    Route::get('/invition-for-hire-freelancer/{username}', 'HireController@freelancer_invition')->name('invition_for_hire_freelancer')->middleware(['Maintenance']);
    Route::post('/invition-for-hire-freelancer/store', 'HireController@store')->name('invition_for_hire_freelancer_sent');

    //Milestone payment
    Route::get('/recieved-milestone-requests', 'MilestonePaymentController@recieved_milestone_request_index')->name('milestone-requests.all')->middleware(['Maintenance']);
    Route::post('/milestone-payment-select-modal', 'MilestonePaymentController@show_payment_select_modal')->name('show_payment_select_modal')->middleware(['Maintenance']);
...
Route::group(['middleware' => ['auth', 'freelancer', 'packagePurchased']], function () {
    ...
    Route::get('/account-settings', 'ProfileController@user_account')->name('user.account')->middleware(['Maintenance']);;

    Route::post('/profile-settings/portfolio-add', 'PortfolioController@store')->name('user_profile.portfolio_add');
    Route::get('/profile-settings/portfolio-edit/{id}', 'PortfolioController@edit')->name('user_profile.portfolio_edit')->middleware(['Maintenance']);
    Route::post('/profile-settings/portfolio-update/{id}', 'PortfolioController@update')->name('user_profile.portfolio_update');
    Route::get('/profile-settings/portfolio-delete/{id}', 'PortfolioController@destroy')->name('user_profile.portfolio_destroy')->middleware(['Maintenance']);
...
Route::get('/search', 'SearchController@index')->name('search')->middleware(['Maintenance']);
